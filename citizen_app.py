...
class CitizenApp(ctk.CTk):
    ...
    def create_compact_form_field(self, parent, label_text, placeholder, row, col):
    ...
    def show_login_view(self, prefill_id=""):
        ...
        self.login_id_entry = self.styled_entry(
            card,
            "e.g. 10"
        )

        self.login_id_entry.grid(
            row=3,
            column=0,
            padx=36,
            sticky="ew"
        ...
        self.login_pw_entry = self.styled_entry(
            card,
            "Password",
            show="•"
        )

        self.login_pw_entry.grid(
            row=5,
            column=0,
            padx=36,
            sticky="ew"
        ...
        self.login_pw_entry.bind(
            "<Return>",
            lambda e: self.attempt_login()
        )
    ...
    def show_submit_complaint_view(
        self,
        edit_id=None,
        orig_title="",
        orig_description="",
        orig_dept="",
        orig_priority="Medium"
    ):

        self.clear_main_frame()

        self.editing_id = edit_id

        page_title = "Edit Complaint" if edit_id else "Submit a Complaint"

        page_sub = (
            f"Editing Complaint #{edit_id}"
            if edit_id
            else "Tell us about the issue and which department it concerns."
        )
        ...
        # Scrollable form so all fields and the button remain accessible
        ...
        description_frame.grid_columnconfigure(
            0,
            weight=1
        )
        ...
        self.entry_comp_description = ctk.CTkTextbox(
            description_frame,
            height=110,
            fg_color=BG_CARD,
            border_color=BORDER,
        ...
        self.entry_comp_description.grid(
            row=1,
            column=0,
            sticky="ew"
        )
        ...
        if orig_description:
            self.entry_comp_description.insert(
                "1.0",
                orig_description
            )

        # Placeholder-like helper text
        if not orig_description:
            self.entry_comp_description.insert(
                "1.0",
                "Briefly describe the issue, including the location and any important details."
            )
            self.entry_comp_description.configure(
                text_color=TEXT_MUTED
            )

            def clear_description_placeholder(event=None):

                current_text = self.entry_comp_description.get(
                    "1.0",
                    tk.END
                ).strip()

                if current_text == (
                    "Briefly describe the issue, including the location "
                    "and any important details."
                ):

                    self.entry_comp_description.delete(
                        "1.0",
                        tk.END
                    )
            ...
            def restore_description_placeholder(event=None):
            ...
            self.entry_comp_description.bind(
                "<FocusIn>",
                clear_description_placeholder
            )

            self.entry_comp_description.bind(
                "<FocusOut>",
                restore_description_placeholder
            )
    ...
    def insert_complaint(self):
        ...
        description = self.entry_comp_description.get(
            "1.0",
            tk.END
        ).strip()
        ...
        # Remove placeholder if user didn't type anything
        placeholder = (
            "Briefly describe the issue, including the location "
            "and any important details."
        )
    ...
    def show_track_view(self):
        ...
        for col in columns:
            ...
            align = (
                "center"
                if col in ("ID", "Status", "Priority", "Date")
                else "w"
            )
        ...
        ctk.CTkButton(
            ...
            font=(FONT, FONT_BUTTON, "bold"),
            corner_radius=9,
            command=self.edit_selected_complaint
        ).pack(
            side="left",
            padx=(0, 10)
        )
        ...
        ctk.CTkButton(
            ...
            font=(FONT, FONT_BUTTON, "bold"),
            corner_radius=9,
            command=self.delete_selected_complaint
        ).pack(
            side="left"
        )
    ...
    def delete_selected_complaint(self):
        ...
        if not selected:
            ...
            messagebox.showwarning(
                "Selection Required",
                "Please select a complaint row to delete."
            )

            return

        item = self.track_tree.item(
            selected[0]
        )
        ...
        confirm = messagebox.askyesno(
            "Confirm Deletion",
            f"Are you sure you want to delete Complaint #{complaint_id}?\n\n"
        ...
        try:
            ...
            messagebox.showinfo(
                "Success",
                f"Complaint #{complaint_id} deleted successfully."
            )
    ...
    def edit_selected_complaint(self):
        ...
        if not selected:
            ...
            messagebox.showwarning(
                "Selection Required",
                "Please select a complaint row to edit."
            )
        ...
        try:
            ...
            if row:
                ...
                title, description, dept_id, priority = row
            ...
            messagebox.showerror(
                "Database Error",
                f"Failed to fetch complaint details:\n{e}"
            )
